...
* **Tools:** `ab` (ApacheBench) for web server load testing, `fio` for disk read throughput, `blockdev` for kernel tuning, and `htop` for live monitoring.
* **Procedure:**
...
**Objective:** Verify system resource saturation (CPU/RAM) during load testing to identify bottlenecks.
...
The following chart illustrates the relative improvement in processing power (Nginx) and data throughput (Disk I/O) after optimization.

```text
Requests Per Second (Higher is better)
Baseline :  [██████████          ] [ENTER BASELINE NUMBER]
Optimized:  [██████████████      ] [ENTER OPTIMIZED NUMBER]
